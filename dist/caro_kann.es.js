...
const v = ({ storageKey: e, storageType: r, migrate: t }) => {
  ...
  if (!/=>/.test(e)) throw new Error('Invalid caro-kann selector format: missing " => "');
  if (/{|}/.test(e)) throw new Error("Invalid caro-kann selector format: contains curly braces({ })");
  if (/&|:|\?/.test(e)) throw new Error("Invalid caro-kann selector format: contains disallowed special characters(? : &)");
  const r = e.split("=>")[1].trim(), t = r.match(/\[(?!["'])([^\]]+)(?!["'])\]/);
  if (t) throw new Error(`Invalid path detected: ${t[0]}`);
